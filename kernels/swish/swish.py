...
def torch_swish(x: torch.Tensor, out: torch.Tensor) -> torch.Tensor:
    """x * sigmoid(x) 参考实现，结果写入 out。"""
    out.copy_(x * torch.sigmoid(x))
    return out


def torch_swish_ref(x: torch.Tensor) -> torch.Tensor:
    """纯函数版本，用于生成参考值。"""
...
def run_multi_shape_precision_suite(seed: int = 12345) -> None:
    ...
    print("=" * 85)
    print("精度校验: 自定义 swish vs torch 参考实现（多 shape）")
    print("=" * 85)
    failed = 0
