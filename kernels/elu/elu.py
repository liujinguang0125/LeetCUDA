...
def torch_elu(x: torch.Tensor, out: torch.Tensor) -> torch.Tensor:
    """与 elu.cu 中 ALPHA=1.0 等价的参考实现。"""
    result = torch.where(x > 0, x, torch.exp(x) - 1)
    out.copy_(result)
    return out
...
def run_multi_shape_precision_suite(seed: int = 12345) -> None:
    ...
    print("=" * 85)
    print("精度校验: 自定义 elu vs torch 参考实现（多 shape）")
    print("=" * 85)
    failed = 0
    for shape in PRECISION_SHAPES:
        ...
        ref_f32 = torch.where(x > 0, x, torch.exp(x) - 1)
        ...
        ref_f16 = torch.where(x16 > 0, x16, torch.exp(x16.float()).half() - 1)
