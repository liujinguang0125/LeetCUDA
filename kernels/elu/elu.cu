...
// ELU 计算函数
// FP32: __expf 替代 expf，精度略低但吞吐更高（已开 --use_fast_math 时效果等价）
__device__ __forceinline__ float elu(float x) {
  return x > 0.f ? x : ALPHA * (__expf(x) - 1.f);
}

// FP16: ALPHA==1.0 时省去乘法
__device__ __forceinline__ half elu_half(half x) {
  return __hgt(x, __float2half(0.f)) ? x : __hsub(hexp(x), __float2half(1.f));
}

// CUDA 核函数
// FP32
__global__ void elu_f32_kernel(float *__restrict__ x, float *__restrict__ y,
                               int N) {
...
__global__ void elu_f32x4_kernel(float *__restrict__ x, float *__restrict__ y,
                                 int N) {
...
// FP16
__global__ void elu_f16_kernel(half *__restrict__ x, half *__restrict__ y,
                               int N) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < N)
...
__global__ void elu_f16x2_kernel(half *__restrict__ x, half *__restrict__ y,
                                 int N) {
...
__global__ void elu_f16x8_kernel(half *__restrict__ x, half *__restrict__ y,
                                 int N) {
...
__global__ void elu_f16x8_pack_kernel(half *__restrict__ x,
