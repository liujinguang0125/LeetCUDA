...
def torch_hardswish(x: torch.Tensor, out: torch.Tensor) -> torch.Tensor:
    out.copy_(F.hardswish(x))
    return out


def torch_hardswish_ref(x: torch.Tensor) -> torch.Tensor:
...
def run_multi_shape_precision_suite(seed: int = 12345) -> None:
    ...
    print("=" * 85)
    print("精度校验: 自定义 hardswish vs F.hardswish（多 shape）")
    print("=" * 85)
    failed = 0
